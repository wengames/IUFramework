...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    id obj = [NSNull null];
    NSLog(@"obj[0] = %@", obj[0]);
    NSLog(@"[[NSNull null] integerValue] = %ld", (long)[obj integerValue]);
    NSLog(@"[[NSNull null] length] = %ld", (long)[obj length]);
    NSLog(@"[[NSNull null] count] = %ld", (long)[obj count]);
    ...
    tabBarController.viewControllers = @[
                                         [[[UINavigationController alloc] initWithRootViewController:[[IndexViewController alloc] init]] configure:^(UINavigationController *x) {
                                             [x setPopDuration:1];
                                             [x setPopType:IUNavigationPopTypeErase];
                                             x.tabBarItem.title = @"erase";
                                         }],
                                         [[[UINavigationController alloc] initWithRootViewController:[[IndexViewController alloc] init]] configure:^(UINavigationController *x) {
                                             x.tabBarItem.title = @"default";
                                         }]
                                         ];
